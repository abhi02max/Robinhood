...
function checkProblem(pr, p) {
  ...
  if (!isPlainObject(pr.starter_code)) {
    ...
    // `cpp` is optional but permitted: the C++ harness takes its shape from
    // `cpp_signature`, so a starter is a courtesy to the user rather than a
    // requirement. It is generated by server/scripts/backfill-cpp-starters.js.
    const codeAllowed = new Set(['javascript', 'python', 'cpp']);
    ...
    if ('cpp' in pr.starter_code && !isString(pr.starter_code.cpp, 10)) {
      errs.push(`${p}.starter_code.cpp: must be a string of >= 10 chars when present`);
    }
    // A C++ starter with no signature behind it is a trap: the editor shows a
    // function to fill in and the submission is then rejected with "no
    // cpp_signature configured".
    if ('cpp' in pr.starter_code && !('cpp_signature' in pr)) {
      errs.push(`${p}.starter_code.cpp: present without cpp_signature — C++ submissions would be rejected`);
    }
  }
